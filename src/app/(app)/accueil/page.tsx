...
export default async function AccueilPage() {
  ...
  const lang: Lang = 'fr'
  const isAutoUsername = (u?: string | null) => !u || /^[0-9a-f-]{8,}$/i.test(u) || u.startsWith('user_')
  const memberName = profile?.displayName || (isAutoUsername(profile?.username) ? 'Explorateur' : profile?.username) || 'Explorateur'
